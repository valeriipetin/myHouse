...
class SearchDmViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func fetchPosts(){
        ...
        if (FIRAuth.auth()?.currentUser?.uid) != nil{
            ...
            ref.child("Аренда").child("Дома").queryOrderedByKey().observeSingleEvent(of: .value, with: { (snap) in
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "postCellSnyatDom", for: indexPath) as! PostSnyatDomCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        self.performSegue(withIdentifier:"showDetailSnyatDom", sender: indexPath)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetailSnyatDom" {
            if let vc = segue.destination as? PreviewDmTableViewController {
