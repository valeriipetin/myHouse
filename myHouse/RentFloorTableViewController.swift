//
//  RentFloorTableViewController.swift
//  myHouse
//
...
class RentFloorTableViewController: UITableViewController {
    ...
    var selectedFloor:String? {
        didSet {
            if let floor = selectedFloor {
                selectedFloorIndex = floors.index(of: floor)
            }
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "SFloorCell", for: indexPath)
        cell.textLabel?.text = floors[indexPath.row]
        
        //выбор элемента (галочка)
        if indexPath.row == selectedFloorIndex {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let index = selectedFloorIndex {
            let cell = tableView.cellForRow(at: IndexPath(row: index, section: 0))
            cell?.accessoryType = .none
        }
        
        selectedFloor = floors[indexPath.row]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "SSaveSelectedFloor" {
